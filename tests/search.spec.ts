...
test('Realizar una busqueda que no tenga resultados', async ({ page }) => {
  ...
  await expect(page.locator('.DocSearch-NoResults p.DocSearch-Title')).toBeVisible({timeout: 5000});

  await expect(page.locator('.DocSearch-NoResults p.DocSearch-Title')).toHaveText('No results for "hascontent"');
